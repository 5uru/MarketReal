...
for i in range(119):
    ...
    results = soup.find_all("a", class_="card-image ad__card-image waves-block waves-light")
...
with open('terrain_coinafrique.txt', 'r') as file:
    for line in file:
        try:
            ...
            description = soup.find("div", class_="ad__info__box ad__info__box-descriptions").text
            # save on csv file
            with open('terrain_coinafrique.csv', 'a') as file_csv:
                file_csv.write(f"{title}, {price}, {superficie}, {description}\n")
        except Exception:
